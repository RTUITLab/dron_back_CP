...
func (qq *QuestionQuery) QueryTheoreticalTest() *TheoreticalTestQuery {
	query := &TheoreticalTestQuery{config: qq.config}
...
func (qq *QuestionQuery) WithTheoreticalTest(opts ...func(*TheoreticalTestQuery)) *QuestionQuery {
	query := &TheoreticalTestQuery{config: qq.config}
	for _, opt := range opts {
		opt(query)
	}
	qq.withTheoreticalTest = query
	return qq
}
...
//		GroupBy(question.FieldTheoricalTestID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Question.Query().
//		Select(question.FieldTheoricalTestID).
//		Scan(ctx, &v)
//
...
func (qq *QuestionQuery) sqlAll(ctx context.Context) ([]*Question, error) {
	...
	if query := qq.withTheoreticalTest; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Question)
		for i := range nodes {
			fk := nodes[i].TheoricalTestID
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "theorical_test_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := qq.withAnswer; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Answer = []*Answer{}
		}
		query.Where(predicate.Answer(func(s *sql.Selector) {
			s.Where(sql.InValues(question.AnswerColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
