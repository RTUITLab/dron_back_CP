...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withUser; query != nil {
		...
		for _, n := range neighbors {
			fk := n.role_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "role_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_user" returned %v for node %v`, *fk, n.ID)
			}
