...
func (tq *TaskQuery) QueryTest() *PractTestQuery {
...
func (tq *TaskQuery) WithTest(opts ...func(*PractTestQuery)) *TaskQuery {
	query := &PractTestQuery{config: tq.config}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withTest; query != nil {
		...
		for i := range nodes {
			fk := nodes[i].PractTestID
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(practtest.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "pract_test_id" returned %v`, n.ID)
			}
			for i := range nodes {
