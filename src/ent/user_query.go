...
func (uq *UserQuery) QueryTheoTry() *TheoreticalTryQuery {
	query := &TheoreticalTryQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTheoTry: uq.withTheoTry.Clone(),
...
func (uq *UserQuery) WithTheoTry(opts ...func(*TheoreticalTryQuery)) *UserQuery {
	query := &TheoreticalTryQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withTheoTry = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRole; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_user" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := uq.withTheoTry; query != nil {
		...
		query.Where(predicate.TheoreticalTry(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TheoTryColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
