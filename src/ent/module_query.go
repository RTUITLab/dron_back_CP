...
func (mq *ModuleQuery) QueryModuleDependcies() *ModuleDependciesQuery {
	query := &ModuleDependciesQuery{config: mq.config}
...
func (mq *ModuleQuery) QueryModuleDependOn() *ModuleDependciesQuery {
	query := &ModuleDependciesQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QuerySubModules chains the current query on the "SubModules" edge.
func (mq *ModuleQuery) QuerySubModules() *SubModuleQuery {
	query := &SubModuleQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *ModuleQuery) WithModuleDependcies(opts ...func(*ModuleDependciesQuery)) *ModuleQuery {
	query := &ModuleDependciesQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withModuleDependcies = query
	return mq
}
...
func (mq *ModuleQuery) WithModuleDependOn(opts ...func(*ModuleDependciesQuery)) *ModuleQuery {
	query := &ModuleDependciesQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withModuleDependOn = query
	return mq
}

// WithSubModules tells the query-builder to eager-load the nodes that are connected to
// the "SubModules" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *ModuleQuery) WithSubModules(opts ...func(*SubModuleQuery)) *ModuleQuery {
	query := &SubModuleQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *ModuleQuery) sqlAll(ctx context.Context) ([]*Module, error) {
	...
	if query := mq.withModuleDependcies; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Module)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ModuleDependcies = []*ModuleDependcies{}
		}
		query.Where(predicate.ModuleDependcies(func(s *sql.Selector) {
			s.Where(sql.InValues(module.ModuleDependciesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "dependent_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.ModuleDependcies = append(node.Edges.ModuleDependcies, n)
		}
	}

	if query := mq.withModuleDependOn; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Module)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ModuleDependOn = []*ModuleDependcies{}
		}
		query.Where(predicate.ModuleDependcies(func(s *sql.Selector) {
			s.Where(sql.InValues(module.ModuleDependOnColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "dependent_on_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.ModuleDependOn = append(node.Edges.ModuleDependOn, n)
		}
	}

	if query := mq.withSubModules; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Module)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.SubModules = []*SubModule{}
		}
		query.withFKs = true
		query.Where(predicate.SubModule(func(s *sql.Selector) {
			s.Where(sql.InValues(module.SubModulesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.module_sub_modules
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "module_sub_modules" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
